...
namespace verint_service.Controllers
{
    ...
    public class VerintOnlineFormController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create(VerintOnlineFormRequest model) {
            try
            {
                return Ok(await _verintOnlineFormService.CreateVOFCase(model));
            }
            catch(Exception ex)
            {
        ...
        /// <summary>
        /// Creates a test case - for ease of testing on pre-production enviroments
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        public async Task<IActionResult> TestCreate()
        {
            var test_model = new VerintOnlineFormRequest
            {
                VerintCase = new Case
                {
                    EventCode = 2002573,
                    ...
                    {
                        CustomerReference = "101003293432",
                        Forename = "Elliott",
                        Surname = "Humphreys"
                    }
                },
            ...
            return Ok(await Create(test_model));
        }
